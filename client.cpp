...
void InitHack()
{
	if (!InitalizeHack)
	{
		if (g_pEngine->GetClientVersion() == CSGO_CLIENT_VERSION && lstrcmpA(g_pEngine->GetProductVersionString(), CSGO_VERSION) == 0)
		{
			g_Leis.InitHack();
...
void HUD_Redraw(const char* pszName, unsigned int vguiPanel)
{
	...
	if (nt_strcmp((char*)pszName, MAT_SYSTEM_TOP_PANEL) == 0)
	{
		...
		{
			g_FontEsp.DrawString(3, 3, Color(0, 255, 0), false, CFG_HACK_MESSAGE);
		}
